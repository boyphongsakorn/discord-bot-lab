...
async function countonlinetime() {
    ...
    if ((new Date().getHours() >= 20 && new Date().getDay() >= 1 && new Date().getDay() <= 5) || (new Date().getHours() >= 0 && new Date().getHours() <= 23 && (new Date().getDay() == 0 || new Date().getDay() == 6))) {
        ...
        for (const [key, value] of Object.entries(userlists)) {
            ...
            if ((new Date().getHours() >= 8 && new Date().getMinutes ==58 && new Date().getDay() > 0 && new Date().getDay() < 6) || (new Date().getHours() >= 0 && new Date().getHours() <= 23 && (new Date().getDay() == 0 || new Date().getDay() == 6) && useridlist.includes(key) == false)) {
                if (value.time > 0) {
                    //send message to dm
